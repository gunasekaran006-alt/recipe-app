...
import { getRecipes, deleteRecipe } from '../services/api';
import { toast } from 'react-toastify';
import axios from 'axios'; // 👈 1. Axios ஐ இம்போர்ட் செய்யவும்
...
function Home() {
    ...
    // 2. டிலீட் செய்யப்பட்ட localStorage-க்கு பதிலாக, யூசர் டேட்டாவிலிருந்து ஃபேவரட்ஸை எடுப்பது
    const [favorites, setFavorites] = useState(() => {
        const savedUser = JSON.parse(sessionStorage.getItem('user'));
        return savedUser && savedUser.favorites ? savedUser.favorites.map(String) : [];
    });

    // 3. Database-based toggleFavorite பங்கஷன்
    const toggleFavorite = async (recipeId) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // SessionStorage-ல் உள்ள user டேட்டாவையும் அப்டேட் செய்வது
                const savedUser = JSON.parse(sessionStorage.getItem('user'));
                if (savedUser) {
            ...
            toast.error(error.response?.data?.message || "Please login to manage favorites");
        }
    };
